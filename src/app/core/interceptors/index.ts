 ...
 * you declare interceptors that inspect and transform HTTP requests from your application to
 * the server. The same interceptors may also inspect and transform the server's responses on
 * their way back to the application. Multiple interceptors form a forward-and-backward chain
 * of request/response handlers.
 * 
 * Interceptors can perform a variety of implicit tasks, from authentication to logging,
 * in a routine, standard way, for every HTTP request/response.
 *
 * Providing Interceptors
 * Because interceptors are (optional) dependencies of the HttpClient service, you must
 * provide them in the same injector (or a parent of the injector) that provides HttpClient.
 ...
 * and HttpResponse instance properties are readonly, rendering them largely immutable.
 *  The request is immutable ... you must clone it.
 * Most interceptors transform the outgoing request before passing it to the next interceptor
 * in the chain, by calling next.handle(transformedReq). An interceptor may transform the response
 * event stream as well, by applying additional RxJS operators on the stream returned by
 * next.handle().
 *
 * Modifing Responses
 ...
 *
 * What is multi true?
 * The multi: true option is a required setting that tells Angular that HTTP_INTERCEPTORS
 * is a token for a multiprovider that injects an array of values, rather than a single value.
 *
 * Order is important
 * Angular applies interceptors in the order that you provide them. If you provide interceptors
 * A, then B, then C, requests will flow in A->B->C and responses will flow out C->B->A.
 */
